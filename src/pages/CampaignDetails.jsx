    ...
    <div>
      ...
      <div className='w-full flex md:flex-row flex-col mt-10 gap-[30px] '>
        <div className='flex-1 flex-col ' >
          ...
          <div className='relative w-full h-[5px] bg-[#3a3a43] mt2 '>
        ...
        <div className='flex md:w-[150px] w-full flex-wrap justify-between gap-[30px] '>
          ...
          {state.target <= state.amountCollected ? (
            ...
            <>
              <CountBox title="Days Left" value={remainingDays} isFinish={true} />
              <CountBox title={`Raised of ${state.target}`} value={state.amountCollected} isFinish={true} /> 
              <CountBox title="Total Backers" value={donators?.length} isFinish={true} />
            </>
            ...
            <div className='mt-[20px] flex flex-col gap-4'>
              {donators?.length > 0 ? donators?.map((item, index) => (
                <div key={`${item.donator}-${index}`} className='flex justify-between items-center gap-4'>
                  <p className='font-epilogue font-normal text-[16px] text-[#b2b3db] leading-[26px] break-all'>
                ...
                <p className='mt-[4px] font-epilogue font-normal text-[16px] text-[#808191] 
                leading-[26px] text-justify  '>
                  No donatros yet. Be the first one!
                </p>
              )}
        ...
        <div className='flex-1'>
          ...
          <div className='mt-[20px] flex flex-col p-4 bg-[#1c1c24] rounded-[10px] '>
            ...
            <div className='mt-[30px]'>
              <form onSubmit={handleDonate} className='w-full flex flex-col  ' >
                <input type='number' placeholder='ETH 0.1' step={0.01} value={amount}
                ...
                <div className='my-[20px] p-4 bg-[#13131a] rounded-[10px] '>
                  ...
                  <p className='font-epilogue font-normal leading-[22px] text-[#808191]'>
                    Suport the project for no reward, just because it speaks to you.
                  </p>
                </div>
                {state.target >= state.amountCollected ? (
                  <CustomButton btnType="submit" title="Fund Campaign" styles="w-full bg-[#8c6dfd]" 
                   />
                ) : (
                  <CustomButton btnType="submit" title="Fund Campaign" styles="w-full bg-[#5b5b5f]" 
                  disabled={true}  />
                )}
